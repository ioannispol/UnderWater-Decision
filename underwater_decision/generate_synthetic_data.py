...
def determine_cleaning_method(hard_perc, hard_mm, soft_perc, soft_mm):
    ...
    if (
        hard_perc >= HARD_PERCENTAGE_THRESHOLD
        and soft_perc >= SOFT_PERCENTAGE_THRESHOLD
    ):
...
def area_coverage_by_fouling_and_depth(
    hard_perc: int, soft_perc: int, depth: int
) -> int:
    """
...
def initialize_and_normalize_weights(real_df):
    ...
    return (
        adjust_and_normalize_weights(hard_perc_weights, 101),
...
def select_fouling_characteristics(
    hard_perc_weights,
    hard_mm_weights,
    soft_perc_weights,
    soft_mm_weights,
...
def generate_single_entry(
    ...
    hard_perc, hard_mm, soft_perc, soft_mm = select_fouling_characteristics(
        hard_perc_weights,
        hard_mm_weights,
        soft_perc_weights,
        soft_mm_weights,
    ...
    total_area_coverage = area_coverage_by_fouling_and_depth(
        hard_perc, soft_perc, avg_depth
    )
...
def generate_synthetic_data(real_df, num_entries):
    ...
    (
        ...
        soft_perc_weights,
        soft_mm_weights,
    ) = initialize_and_normalize_weights(real_df)
...
def get_args():
    ...
    parser = argparse.ArgumentParser(
        description="Generate a synthetic dataset based on real data."
    )
    parser.add_argument(
        "--csv_file_path",
        type=str,
...
if __name__ == "__main__":
    ...
    SHALLOW_DEPTH_COVERAGE_RANGE = tuple(
        config["coverage_percentages"]["shallow_coverage_range"]
    )
    DEEP_DEPTH_COVERAGE_RANGE = tuple(
        config["coverage_percentages"]["deep_coverage_range"]
    )
    GENERIC_COVERAGE_RANGE = tuple(
        config["coverage_percentages"]["generic_coverage_range"]
    )
