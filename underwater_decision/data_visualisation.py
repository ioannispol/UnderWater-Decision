...
def plot_data(csv_path):
    ...
    colors = list(mcolors.TABLEAU_COLORS)  # This gives a list of visually distinct colors

    # Create a figure with two subplots
    fig, (ax_scatter, ax_bar) = plt.subplots(2, 1, figsize=(12, 16), gridspec_kw={'height_ratios': [3, 1]})

    # Scatter plots in the first subplot
    for column, color in zip(columns_to_plot, colors):
        ax_scatter.scatter(data[column], data['Total_Area_Coverage'], alpha=0.5, label=f'{column} vs Total Area Coverage', color=color)
    
    ax_scatter.set_title('Scatter Plots of Fouling Characteristics vs Total Area Coverage')
