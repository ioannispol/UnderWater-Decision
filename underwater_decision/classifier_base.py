...
def encode_columns(df, columns):
    ...
    df = df.apply(
        lambda col: encoders[col.name].transform(col) if col.name in encoders else col
    )
    return df, encoders
...
def get_decision_path(model, features_test, sample_id):
    ...
    for node_id in node_index:
        if (
            features_test.iloc[sample_id, model.tree_.feature[node_id]]
        ...
        rules += (
            f"decision node {node_id} : (X[{sample_id}, {feature_names[model.tree_.feature[node_id]]}] = "
