...
def plot_feature_importance(model, feature_columns, file_path):
    ...
    xgb.plot_importance(model, ax=ax, importance_type='weight', max_num_features=len(feature_columns))
...
def get_args():
    ...
    parser.add_argument("--performance_file", "-p", type=str, default="xgboost_performance.txt",
                        help="Filename for the model's performance metrics.")
    parser.add_argument("--feature_importance_file", "-fi", type=str, default="feature_importance.png",
                        help="Filename for the feature importance plot.")
    parser.add_argument("--tree_plot_file", "-tp", type=str, default="xgb_tree.png",
                        help="Filename for the XGBoost tree plot.")
    parser.add_argument("--learning_curve_file", "-lc", type=str, default="learning_curve.png",
...
if __name__ == "__main__":
    ...
    if args.select_all:
        ...
        xgb_model, best_params, best_score = train_xgboost_with_grid_search(features_train, target_train)

        # Plot Feature Importance
        plot_feature_importance(xgb_model, feature_columns, str(args.feature_importance_file))
